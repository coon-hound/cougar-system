...
| [PIPELINE.md](PIPELINE.md) | How a change ships: worktree per feature, verify gate, PR. |
...
2. **`?v=` cache-buster.** Browsers (and phones especially) cache the JS hard. Every
   ...
   users keep running stale code. Bump it in the same commit as the JS/CSS change.
