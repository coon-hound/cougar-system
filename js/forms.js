  ...
  setTimeout(() => {
    ...
    if (computed.length) {
      ...
      const axisBase = {
        ...
        scales: {
          y: { grid: { color: PC.border }, ticks: { color: PC.muted, font: { size: 9 } } },
          x: { grid: { color: PC.border }, ticks: { color: PC.muted, font: { size: 9 }, maxRotation: 0, autoSkip: true } }
        }
      };
      ...
      new Chart(document.getElementById("pm-hr"), {
        ...
        data: { labels, datasets: [
          { label: "Avg HR", data: computed.map(c => c.avgHr), borderColor: PC.accent, backgroundColor: PC.accentWash, tension: .3, pointRadius: 3 },
          { label: "Max HR", data: computed.map(c => c.maxHr), borderColor: PC.red, backgroundColor: PC.redWash, tension: .3, pointRadius: 3 }
        ] },
        options: { ...axisBase, plugins: { ...axisBase.plugins, legend: { display: true, position: "bottom", labels: { color: PC.muted, font: { size: 9 }, boxWidth: 10 } } } }
      });

      new Chart(document.getElementById("pm-cal"), {
        type: "line",
        data: { labels, datasets: [{ data: computed.map(c => c.calories), borderColor: PC.orange, backgroundColor: PC.orangeWash, fill: true, tension: .3, pointRadius: 3 }] },
        options: axisBase
      });

      new Chart(document.getElementById("pm-eff"), {
        type: "line",
        data: { labels, datasets: [{ data: computed.map(c => c.efficiency), borderColor: PC.teal, backgroundColor: PC.tealWash, fill: true, tension: .3, pointRadius: 3 }] },
        options: axisBase
      });

      new Chart(document.getElementById("pm-int"), {
        type: "line",
        data: { labels, datasets: [{ data: computed.map(c => c.intensity), borderColor: PC.yellow, backgroundColor: PC.yellowWash, fill: true, tension: .3, pointRadius: 3 }] },
        options: { ...axisBase, scales: { ...axisBase.scales, y: { min: 0, max: 100, grid: { color: PC.border }, ticks: { color: PC.muted, font: { size: 9 }, callback: v => v + '%' } } } }
      });

      new Chart(document.getElementById("pm-wl"), {
        type: "bar",
        data: { labels, datasets: [{ data: computed.map(c => c.workload), backgroundColor: PC.purpleWash, borderColor: PC.purple, borderWidth: 1 }] },
        options: axisBase
      });
    }
  }, 100);
...
function buildFitnessReportHTML(d4, startIso, endIso) {
  ...
  // var(--x) does not resolve in a mail client, and these charts are painted
  // onto a white matte (see renderChartPNG), so the app's dark-ground palette
  // would be the wrong palette here even if it could be read. Re-theming the
  // app must not re-theme the recruit's fitness email.
  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = e => {
      ...
      img.onload = () => {
        ...
        const ctx = canvas.getContext("2d");
        // JPEG has no alpha channel, so a transparent source would encode as
        // black. White matte, not a palette colour — leave it literal.
        ctx.fillStyle = "#FFFFFF";
...
async function analyzeAndPushPolarPhotos() {
  ...
  const errorList = errors.length
    ? `<div style="margin-top:12px"><div style="font-size:11px;color:var(--red);text-transform:uppercase;letter-spacing:.5px;margin-bottom:4px">Errors (${errors.length})</div>${errors.map(e => `<div style="font-size:11px;padding:4px 8px;background:rgba(var(--redRGB),.13);border-left:2px solid var(--red);border-radius:3px;margin-bottom:3px"><strong>${escapeAttr(e.photo)}:</strong> ${escapeAttr(e.error)}</div>`).join("")}</div>`
    : "";
