...
- **Just-moved marker + split token** (`MV_SESSION_MOVED`, render.js; populated in `moveToLocation`/`mvUndoLastMove`, forms.js): station ROTATION (Range → Cookhouse → Lecture → Range) is a cycle, so after the first drop every "Pick all here" also grabs the just-arrived bodies and NO drop order avoids it (unlike a chain) - a phone-sim measured the only-correct manual path (per-chip memory audit at each station) at ~51s; the naive path is 7s but silently strands a whole group one station ahead.
  Fix: bodies moved during the current move-mode session carry a ↪ on their chip (`.mv-moved`, teal dashed border), and `mvUpdateSelFrom` splits each location's token into settled vs "✕ ↪ just moved" so one tap sheds the arrivals (rotation is pick-all → shed → drop per station, ~13s, any order).
  The set clears on `enterMoveMode` (session-scoped - staged moves from a PREVIOUS session count as settled originals) and undo/redo keeps it truthful (undo deletes the ids, redo re-adds). `mvDeselectLoc(loc, moved)` narrows to one subgroup; omitted `moved` = whole location.
- Booked-out recruit returning mid-day is ~14s (Dashboard "Currently Out of Camp" row's ↩ Book In, then find-bar + move) - measured within budget, no board-side book-in affordance needed; the board's Out of Camp chips stay deliberately inert.
